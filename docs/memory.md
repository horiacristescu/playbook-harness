...
A hook appends every human message to `.agent/chat_log.md`, so the log
preserves what the human asked, corrected, rejected, and cared about while the
project developed.

This matters because intent is rarely written down once. It becomes clearer
through examples, and through the moments when the human says that something
is wrong. The chat log is evidence, not a finished description of the project.

## Tasks remember work

Each `task.md` under `.agent/tasks/` remembers one bounded piece of work.
...
it remains as the implementation workbook. That makes it possible to study work
later, comparing what was intended with what was built and with what later
tasks revealed.
...
deeper evidence, and it should help a fresh agent orient quickly without
becoming a copy of every task or conversation. When the map and the project
disagree, the map must change.

## Tests remember expectations

A test is memory with an executable check attached.

It preserves an invariant that would otherwise need to be rediscovered, and it
is especially valuable when the original failure was difficult to reproduce or
when a plausible implementation could silently break the same behavior later.
...
Some facts exist only while a system is running. An API response changes, a
long pipeline fails after an hour, a browser reaches a state that cannot be
reconstructed from source code alone. Useful logs and captured traces make
later debugging possible.
...
durable destination, and the useful question is:
...
procedure may belong in a tool, a disappearing condition in a log, and a
correction to the project model in the mind map. Something the human
repeatedly misunderstands may need a plain explanation instead.

Memory is useful when it changes future work.
